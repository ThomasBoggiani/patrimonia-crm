...
const HONORAIRES_MIN = 10000;     // plancher : honoraires toujours ≥ 10 000 €
...
export function getCommission(m) {
  ...
  if (nv && nv > 0 && nv < ttc) return Math.max(0, ttc - nv); // commission réelle connue

  // Estimation par défaut (rien de saisi) : plancher à 10 000 €.
  return Math.max(Math.round(ttc * COMMISSION_FALLBACK / (1 + COMMISSION_FALLBACK)), HONORAIRES_MIN);
}

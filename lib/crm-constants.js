...
export const TYPES_ACTIF_TREE = {
  'Immeubles': ['Mixte', 'Tertiaire', 'Habitation'],
...
// ─────────────────────────────────────────────────────────
// RÉFÉRENTIEL GÉOGRAPHIQUE STRUCTURÉ (socle matching réciproque)
// Chaque zone = un secteur nommé + un test d'appartenance par code postal.
// matchCP : préfixe(s) de code postal couverts par le secteur.
// premium : zones nommées à cheval (codes postaux explicites).
// ─────────────────────────────────────────────────────────
export const ZONES_TREE = {
  'Paris intra-muros': { cp: ['75'] },
  'Hauts-de-Seine (92)': { cp: ['92'] },
  'Seine-Saint-Denis (93)': { cp: ['93'] },
  'Val-de-Marne (94)': { cp: ['94'] },
  'Petite couronne': { cp: ['92', '93', '94'] },
  'Yvelines (78)': { cp: ['78'] },
  'Essonne (91)': { cp: ['91'] },
  "Val-d'Oise (95)": { cp: ['95'] },
  'Seine-et-Marne (77)': { cp: ['77'] },
  'Grande couronne': { cp: ['77', '78', '91', '95'] },
  'Île-de-France': { cp: ['75', '77', '78', '91', '92', '93', '94', '95'] },
  'Province': { exclude: ['75', '77', '78', '91', '92', '93', '94', '95'] }, // France SAUF Île-de-France   'France entière': { cp: [] }, // [] = pas de filtre, tout passe
};

// Zones premium nommées (à cheval sur plusieurs arrondissements/communes)
// Tu les enrichiras au fil de l'eau.
export const ZONES_PREMIUM = {
  // "Triangle d'or": ['75008', '75016', '75017'],
};

// Liste des secteurs sélectionnables (pour le sélecteur unique client/mandat)
export const ZONES_SECTEURS = Object.keys(ZONES_TREE);

// Teste si un code postal appartient à une zone nommée
export function cpInZone(cp, zoneName) {
  if (!cp) return false;
  const cpStr = String(cp).trim();
  // Zone premium ?
  if (ZONES_PREMIUM[zoneName]) {
    return ZONES_PREMIUM[zoneName].includes(cpStr);
  }
  const zone = ZONES_TREE[zoneName];
  if (!zone) return false;
  // Zone d'exclusion (ex: Province = France sauf IDF)
  if (zone.exclude) return !zone.exclude.some(prefix => cpStr.startsWith(prefix));
  if (!zone.cp || zone.cp.length === 0) return true; // France entière
  return zone.cp.some(prefix => cpStr.startsWith(prefix));
}

// Teste si un code postal matche AU MOINS UNE des zones du client
export function cpMatchesAnyZone(cp, clientZones) {
  if (!Array.isArray(clientZones) || clientZones.length === 0) return true; // pas de filtre client
  return clientZones.some(z => cpInZone(cp, z));
}
...
export function getCoverPhoto(mandat) {
  ...
  if (photos.length === 0) {
    // Pas de photo → on retombe sur la façade Street View (déjà récupérée à
    // partir de l'adresse). Mieux qu'une vignette vide.
    return mandat?.street_view_image_url || mandat?.streetViewImageUrl || null;
  }
  const cover = photos.find(p => p.cover);
  return (cover || photos[0]).url || null;
}

// La couverture est-elle la façade Street View par défaut (aucune vraie photo) ?
export function isCoverStreetView(mandat) {
  return getPhotos(mandat).length === 0
    && !!(mandat?.street_view_image_url || mandat?.streetViewImageUrl);
}

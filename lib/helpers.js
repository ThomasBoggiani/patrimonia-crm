// ═══════════════════════════════════════════════════════════════════
// lib/pdf/helpers.js
// Fonctions utilitaires de formatage pour les PDFs
// ═══════════════════════════════════════════════════════════════════
...
export function formatPrix(prix, options = {}) {
  ...
  return `${formatter.format(num)} €${options.suffix || ''}`;
}
...
export function formatSurface(surface) {
  ...
  if (isNaN(num)) return '—';
  return `${new Intl.NumberFormat('fr-FR').format(num)} m²`;
}
...
// ─────────────────────────────────────────────────────────────────
// Reproduit le style "HOTEL A VENDRE - LA CIOTAT - 4 200 000 €" de la plaquette

export function buildTitleCommercial(mandat) {
  const parts = [];

  // Type en majuscules + "À VENDRE"
  if (mandat.type) {
...
// ─────────────────────────────────────────────────────────────────
// La structure photos peut être :
//   - [] (vide)
//   - ["url1", "url2"] (array de strings)
//   - [{ url: "...", caption: "..." }] (array d'objets)
// On normalise vers ["url1", "url2"]
...
// ─────────────────────────────────────────────────────────────────
// Plaquette : 6 photos par page (3 lignes de 2)
...
// ─────────────────────────────────────────────────────────────────
// VÉRIFICATION URL ABSOLUE (pour @react-pdf/renderer qui exige des URLs absolues)
// ─────────────────────────────────────────────────────────────────

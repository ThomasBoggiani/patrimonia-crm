...
import { computeRendements } from '@/lib/rendements'; 
import { cpMatchesAnyZone, cpInZone } from '@/lib/crm-constants';
...
// Référentiel aligné sur crm-constants.js (TYPES_ACTIF_TREE)
// B2C = famille "Résidentiel" + ses sous-types. B2B = tout le reste.
const B2C_TYPES = ['Résidentiel', 'Appartements', 'Maison', 'Hôtels particuliers',
                   // tolérance legacy (anciens libellés au singulier)
                   'Appartement', 'Hôtel particulier'];
const B2B_FAMILLES = ['Immeubles', 'Hôtels', 'Terrains', 'Parking', 'Locaux commerciaux'];
const B2B_SOUS_TYPES = [
  'Mixte', 'Tertiaire', 'Habitation',
...
function checkType(mandat, client) {
  ...
  // Famille B2C : si le client cherche "Résidentiel" (famille), tout sous-type résidentiel matche
  const residentielSousTypes = ['appartements', 'maison', 'hotels particuliers', 'appartement', 'hotel particulier'];
  const clientCherchResidentiel = clientTypes.some(t => norm(t) === 'residentiel');
  const mandatEstResidentiel = candidates.some(c => residentielSousTypes.includes(c));

  let matchedClient = clientTypes.find(t => {
    const tNorm = norm(t);
    return candidates.some(c => c.includes(tNorm) || tNorm.includes(c));
  });
  if (!matchedClient && clientCherchResidentiel && mandatEstResidentiel) {
    matchedClient = 'Résidentiel';
  }
...
function checkZone(mandat, client) {
  ...
  // Code postal du mandat : champ dédié, sinon extrait de l'adresse/ville
  let cp = mandat.codePostal || mandat.code_postal || mandat.cp || '';
  if (!cp) {
    const loc = [mandat.adresse, mandat.ville].filter(Boolean).join(' ');
    const m = loc.match(/\b(\d{5})\b/);
    if (m) cp = m[1];
  }

  // Pas de CP exploitable : on ne bloque PAS (champ vide = on laisse passer),
  // pour ne pas éliminer un mandat juste parce que son CP n'est pas encore saisi.
  if (!cp) return { score: WEIGHTS.zone, ok: true, raison: 'Localisation à préciser' };

  if (cpMatchesAnyZone(cp, clientZones)) {
    const matched = clientZones.find(z => cpInZone(cp, z));
    return { score: WEIGHTS.zone, ok: true, raison: `Zone "${matched}" (CP ${cp})` };
  }

  return { score: 0, ok: false, raison: `CP ${cp} hors zones (${clientZones.join(', ')})` };
}
...
function checkRendement(mandat, client) {
  ...
  const rdtPresent = num(_rdt.actuel);
  const rdtOptimise = num(_rdt.optimise);

...
export function computeRendements(mandat) {
  ...
  // ─── Fallback : pas d'état locatif détaillé mais un loyer annuel global saisi ───
  // (loyers_annuels est ANNUEL, pas besoin de ×12)
  if (calcActuel == null && prixNet > 0) {
    const loyerAnnuel = parseFloat(mandat.loyers_annuels || mandat.loyersAnnuels || 0);
    if (loyerAnnuel > 0) calcActuel = Math.round((loyerAnnuel / prixNet) * 1000) / 10;
  }
...
export function computeRendementsAuto(mandat) {
  ...
  if (!Array.isArray(lots) || lots.length === 0) {
    // Pas d'état locatif détaillé : on calcule le rendement actuel depuis le
    // loyer annuel global s'il est renseigné (loyers_annuels est ANNUEL).
    const loyerAnnuel = parseFloat(mandat.loyers_annuels || mandat.loyersAnnuels || 0);
    return {
      actuel: loyerAnnuel > 0 ? Math.round((loyerAnnuel / prixNet) * 1000) / 10 : null,
      optimise: null,
    };
  }
...
export function totalLoyerMensuelOptimise(lots) {
  ...
  return lots.reduce((sum, lot) => {
    const opt = parseFloat(lot.loyer_potentiel || lot.loyerPotentiel || lot.loyer_optimise || lot.loyerOptimise || 0);

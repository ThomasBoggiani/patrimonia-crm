...
export function googleSatelliteUrl({ lat, lng, zoom = 18, width = 600, height = 400 }) {
  // Maptiler satellite (vraie vue satellite, fonctionne en EU)
  // 100k tiles/mois gratuits
  const apiKey = 3sSbFtNdrbvQH1IqKh4h;
  const z = Math.min(zoom, 18);
  return `https://api.maptiler.com/maps/satellite/static/${lng},${lat},${z}/${width}x${height}@2x.jpg?key=${apiKey}`;
}

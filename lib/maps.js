...
// ─────────────────────────────────────────────────────────
export function cadastreUrl({ lat, lng, width = 1000, height = 660, zoom = 18, bbox = null }) {
  // Centre : centroïde de la parcelle si dispo, sinon l'adresse géocodée.
  const cLat = bbox ? (bbox.minLat + bbox.maxLat) / 2 : lat;
  const cLng = bbox ? (bbox.minLng + bbox.maxLng) / 2 : lng;
  const aspect = width / height;
  const cosLat = Math.cos((cLat * Math.PI) / 180) || 0.7;

  // Vue cible ~260 m de haut : on voit le quartier, la parcelle est bien lisible
  // sans remplir tout le cadre (fini le zoom extrême sur « 0119 »).
  let halfLat = 130 / 110540;
  if (bbox) {
    // Si la parcelle est plus grande que la vue par défaut, on l'englobe avec marge.
    const parcelHalfLat = (bbox.maxLat - bbox.minLat) / 2;
    halfLat = Math.max(halfLat, parcelHalfLat * 2.6);
  }
  // Largeur calculée pour respecter le ratio de l'image (pas de déformation).
  const halfLng = (halfLat * aspect) / cosLat;

  const minLat = cLat - halfLat;
  const maxLat = cLat + halfLat;
  const minLng = cLng - halfLng;
  const maxLng = cLng + halfLng;

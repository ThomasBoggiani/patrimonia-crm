...
export function googleStreetViewUrl({ lat, lng, width = 640, height = 400, heading = null, fov = 80, pitch = 10 }) {
  ...
  // heading optionnel : si non précisé, Google choisit le meilleur angle
...
export async function getCadastreParcelle({ lat, lng }) {
  ...
  try {
    ...
    return {
     export function cadastreUrl({ lat, lng, width = 600, height = 400, zoom = 18, bbox = null }) {   // Si on a une bbox précise de la parcelle, on l'utilise (avec un padding de 30%)   let minLat, maxLat, minLng, maxLng;   if (bbox) {     const padLat = (bbox.maxLat - bbox.minLat) * 0.4;     const padLng = (bbox.maxLng - bbox.minLng) * 0.4;     minLat = bbox.minLat - padLat;     maxLat = bbox.maxLat + padLat;     minLng = bbox.minLng - padLng;     maxLng = bbox.maxLng + padLng;   } else {     // Fallback : carré centré sur le point     const span = 0.0008;     minLat = lat - span;     maxLat = lat + span;     minLng = lng - span;     maxLng = lng + span;   }    const params = new URLSearchParams({     SERVICE: 'WMS',     VERSION: '1.3.0',     REQUEST: 'GetMap',     LAYERS: 'CADASTRALPARCELS.PARCELLAIRE_EXPRESS',     STYLES: '',     CRS: 'EPSG:4326',     BBOX: `${minLat},${minLng},${maxLat},${maxLng}`,     WIDTH: String(width),     HEIGHT: String(height),     FORMAT: 'image/png',     TRANSPARENT: 'false'   });    return `https://data.geopf.fr/wms-r/wms?${params.toString()}`; }
      ...
      bbox, // { minLng, maxLng, minLat, maxLat } pour cadrer le cadastre
    };
  } catch (e) {
...
export async function getNearbyTransports({ lat, lng, radius = 500 }) {
  ...
  try {
    // Timeout 8s pour ne pas bloquer la g\u00e9n\u00e9ration du PDF

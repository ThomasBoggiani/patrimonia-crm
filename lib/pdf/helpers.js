...
// ─────────────────────────────────────────────────────────────────
// SÉLECTION DES PHOTOS POUR LA PLAQUETTE
// Si au moins une photo est cochée « plaquette » → on ne prend que celles-là.
// Sinon → toutes les photos (comportement historique). La couverture est
// toujours incluse et placée en tête (elle sert d'image de couverture).
// ─────────────────────────────────────────────────────────────────
export function selectPhotosForPlaquette(mandat) {
  const photoMedias = Array.isArray(mandat?.medias)
    ? mandat.medias.filter((m) => m && m.type === 'photo')
    : [];

  if (photoMedias.length === 0) return normalizePhotos(mandat); // fallback legacy `photos`

  const selected = photoMedias.filter((m) => m.plaquette === true);
  let pool = selected.length > 0 ? [...selected] : [...photoMedias];

  // Garantir la présence de la couverture en tête
  const cover = photoMedias.find((m) => m.cover);
  if (cover && !pool.some((m) => m.url === cover.url)) pool = [cover, ...pool];

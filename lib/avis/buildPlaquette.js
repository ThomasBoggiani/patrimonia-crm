...
function slCover(d, m) {
  ...
  const photo = d.coverPhoto
    ? `<div style="position:relative;height:700px;overflow:hidden;background:#EDE6DC">
        <img src="${esc(optImg(d.coverPhoto))}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover"/>
...
function slLocalisation(d, n) {
  ...
  const map = L.mapUrl
    ? `<div style="height:420px;position:relative;border:1px solid ${LINE};overflow:hidden;background:#EDE6DC;flex-shrink:0"><img src="${esc(optImg(L.mapUrl))}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover"/></div>`
    : '';

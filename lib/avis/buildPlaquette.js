...
// ── 3. LE BIEN — descriptif COMPLET + chiffres clés (bande compacte) ─────
function slLeBien(d, m, n) {
  const cards = (d.synthese && d.synthese.cards ? d.synthese.cards : []).slice(0, 6);
  // Descriptif COMPLET (jamais tronqué) : priorité au texte du mandat.
  const desc = String(m?.description || d.pitch || '').trim();
  if (!cards.length && !desc) return '';
  // Taille adaptée à la longueur pour tout faire tenir sur la page.
  const fs = desc.length > 1100 ? 15 : desc.length > 800 ? 16 : desc.length > 500 ? 18 : 22;
  const pitch = desc
  ...
  // Chiffres clés en bande compacte (une seule ligne) → laisse la place au texte.
  const stat = (c) => `<div style="flex:1;min-width:0;border-left:1px solid ${LINE2};padding:0 4px 0 14px">
      <div style="font-family:${SERIF};font-size:30px;color:${GREEN};line-height:1">${esc(c.figure)}</div>
      <div style="font-size:9.5px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:${SAGE};margin-top:4px">${esc(c.label)}</div></div>`;
  const strip = cards.length ? `<div style="display:flex;margin-top:24px;flex-shrink:0">${cards.map(stat).join('')}</div>` : '';
  return pPage('Le bien', 'En quelques mots', `${pitch}${strip}`, d, n);
}
...
function slLocalisation(d, n) {
  ...
  const map = L.mapUrl
    ? `<div style="height:280px;position:relative;border:1px solid ${LINE};overflow:hidden;background:#EDE6DC;flex-shrink:0"><img src="${esc(optImg(L.mapUrl))}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover"/></div>`
    : '';
...
function slCadastre(d, n) {
  ...
  const img = L.cadastreImg
    ? `<div style="flex:1;min-height:0;border:1px solid ${LINE};overflow:hidden;background:#EDE6DC;position:relative"><img src="${esc(optImg(L.cadastreImg))}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:contain"/></div>`
    : '';
...
function slEquipe(d, n, conseiller) {
  ...
  const avatar = c.photo
    ? `<img src="${esc(optImg(c.photo))}" alt="" style="width:120px;height:120px;border-radius:50%;object-fit:cover;flex-shrink:0"/>`
...
export function buildPlaquetteHtml(m, opts = {}) {
  ...
  // Ordre : rituel → bien → SECTEUR (emplacement · foncier · risques) → photos
  // par étage / galerie → état locatif → équipe.
  return [
    slCover(d, m),
    slLeBien(d, m, N()),
    slLocalisation(d, N()),

...
function slMarche(d, n) {
  ...
  let evoBlock = '';
  if (E && E.parAnnee.length) {
    const maxV = Math.max(...E.parAnnee.map(a => a.m2Median || 0), 1);
    const bar = (a) => {
      const h = Math.round(40 + 210 * ((a.m2Median || 0) / maxV));
      return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:flex-end;flex:1;gap:8px">
        <div style="font-family:'Cormorant Garamond',serif;font-size:22px;color:#F5EDE5">${fmtNum(a.m2Median)}</div>
        <div style="width:60%;max-width:96px;height:${h}px;background:linear-gradient(180deg,#C3CCB8,#8C9978);border-radius:4px 4px 0 0"></div>
        <div style="font-size:15px;color:rgba(245,237,229,.8)">${esc(a.annee)}</div>
        <div style="font-size:12px;color:rgba(245,237,229,.5)">${a.count} vente(s)</div></div>`;
    };
    const focus = E.focus12;
    evoBlock = `<div style="display:grid;grid-template-columns:1.7fr 1fr;gap:48px;align-items:center;padding-top:22px">
      <div><div style="font-size:14px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#C3CCB8;margin-bottom:14px">Évolution des prix sur 5 ans</div>
        <div style="display:flex;align-items:flex-end;gap:16px;height:280px">${E.parAnnee.map(bar).join('')}</div></div>
      <div style="border-left:1px solid rgba(245,237,229,.2);padding-left:40px">
        ...
        ${focus && Number.isFinite(focus.variationPct) && focus.variationPct !== 0 ? `<div style="margin-top:12px;font-size:20px;color:${focus.variationPct > 0 ? '#C3CCB8' : '#CFBA9E'}">${focus.variationPct > 0 ? '▲' : '▼'} ${Math.abs(focus.variationPct)} % vs période précédente</div>` : ''}
...
export function buildAvisData(m) {
  ...
  // CHECK-LIST des pièces du dossier — cochée auto si la donnée existe, sinon manuelle.
  ...
  const ckItems = [
    ...
    ckItem('type_surface', 'Type & surface du bien', 1, surface),
    ckItem('photos', 'Photos du bien', 2, photos.length),
    ckItem('plans', 'Plans du bien', 2, false),
    ckItem('titre', 'Titre de propriété', 2, false),
    ckItem('dpe', 'DPE', 2, m?.dpe_classe),
    ckItem('diagnostics', 'Diagnostics techniques', 2, doc.diagnostics),
    ckItem('taxe', 'Avis de taxe foncière', 2, taxeF),
    ckItem('charges', 'Charges / pré-état daté', 2, num(doc.charges_annuelles)),
    ckItem('reglement', 'Règlement de copropriété', 2, false),
    ckItem('pv_ag', 'PV des 3 dernières AG', 2, false),
...
export function buildAvisHtml(m) {
  ...
  return [
    ...
    slGalerie(d, N()),
    // Dossier — check-list des pièces (par niveau) puis DPE + copro/fiscalité/diagnostics
    slChecklist(d, N()),
    slDocuments(d, N()),

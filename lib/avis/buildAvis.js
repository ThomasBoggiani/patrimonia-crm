...
export function buildAvisData(m) {
  ...
  // Prix arrondis à 5 000 € près (présentation).
  const bas = round5k(num(preco.prix_plancher) || num(m2v.valeur_basse?.valeur_totale));
  const haut = round5k(num(preco.prix_coup_de_coeur) || num(m2v.valeur_haute?.valeur_totale));
  const central = round5k(num(preco.prix_marche) || num(m2v.valeur_centrale?.valeur_totale));
  ...
  let calcul = null;
  if (dvfM2 && surface) {
    const ref = round5k(dvfM2 * surface);              // arrondi 5 000 €
    const ajs = (preco.ajustements || []).filter(a => a && (a.label || a.pct));
    let cumule = ref;
    const steps = ajs.map(a => {
      const pct = +a.pct || 0;
      const effet = round5k(ref * pct / 100); // additif sur la référence (arrondi 5 000 €)
      cumule += effet;

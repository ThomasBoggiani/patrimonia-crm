...
function slLocalisation(d, n) {
  ...
  const cadastreBloc = (L.cadastreRef || L.cadastreImg) ? `<div style="display:flex;align-items:center;gap:14px;margin-bottom:20px">
    ${L.cadastreImg ? `<div style="width:112px;height:84px;flex-shrink:0;border:1px solid rgba(41,64,41,.2);overflow:hidden;background:#EDE6DC"><img src="${esc(L.cadastreImg)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"/></div>` : ''}
    <div>
      <div style="font-size:13px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:#8C9978">Cadastre</div>
      <div style="font-family:'Cormorant Garamond',serif;font-size:30px;color:#294029;line-height:1.1">${esc(L.cadastreRef || '—')}</div>
      ${L.cadastreSurface ? `<div style="font-size:15px;color:#8C9978">${esc(L.cadastreSurface)}</div>` : ''}</div></div>` : '';
...
function slDocuments(d, n) {
  ...
  const chargesDetail = D.chargesDetail ? `<div style="margin:-6px 0 22px;font-size:16.5px;line-height:1.45;color:rgba(36,25,18,.75)"><b style="color:#8C9978">Les charges comprennent :</b> ${esc(D.chargesDetail)}</div>` : '';
  return section('#F5EDE5',
    `${header('Copropriété, fiscalité & diagnostics', 'Le dossier du bien')}
     <div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding-top:10px">
       ${dpeBand}${stats}${chargesDetail}${textes}

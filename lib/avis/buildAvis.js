...
export function buildAvisData(m) {
  ...
  // Localisation — carte + transports (depuis transports_data)
  const td = m?.transports_data || {};
  const transports = [];
  const pushMode = (arr, label) => (arr || []).slice(0, 4).forEach(s => {
    const nom = s.nom || s.name || s.station || '';
    const dist = s.distance ? ` — ${Math.round(s.distance)} m` : '';
    const lig = s.lignes ? ` (${[].concat(s.lignes).join(', ')})` : (s.ligne ? ` (${s.ligne})` : '');
  ...
  const mapUrl = m?.map_static_image_url || m?.cadastre_image_url || null;
  const localisation = (mapUrl || transports.length || av.localisation?.commentaire) ? {
    mapUrl, transports,
    commodites: av.localisation?.commodites || [],
    loisirs: av.localisation?.loisirs || [],
  ...
  const caracteristiques = cr.length ? { rows: cr, photo: photos[1] || photos[0] || null, description: m?.description || '', title: 'Le bien en détail' } : null;

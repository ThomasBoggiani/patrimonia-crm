  ...
  if (E && E.parAnnee.length >= 2) {
    ...
    evoBlock = `<div style="display:grid;grid-template-columns:1.6fr 1fr;gap:48px;align-items:center;padding-top:24px">
      <div><div style="display:flex;align-items:baseline;gap:14px;margin-bottom:8px">
          <div style="font-size:14px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#C3CCB8">Évolution des prix sur 5 ans</div>
    ...
    `${header('Caractéristiques', C.title || 'Le bien en détail')}
     <div style="flex:1;display:grid;grid-template-columns:1fr 1.1fr;gap:50px;padding-top:16px;align-items:start">
       ...
       <div>${C.rows.map(r => row(r[0], r[1])).join('')}
         ${C.description ? `<div style="margin-top:22px;font-size:19px;line-height:1.55;color:#241912">${esc(C.description)}</div>` : ''}
...
export function buildAvisData(m) {
  ...
  const descTxt = String(avCarac.commentaire || avCarac.distribution || m?.description || '').trim();
  ...
  const ckItems = [
    ...
    ckItem('plans', 'Plans du bien', 2, hasPlan),
    ckItem('titre', 'Titre de propriété', 2, false),
    ckItem('dpe', 'DPE', 2, m?.dpe_classe || num(m?.dpe_consommation)),
    ckItem('diagnostics', 'Diagnostics techniques', 2, doc.diagnostics),
    ckItem('taxe', 'Avis de taxe foncière', 2, taxeF),
    ckItem('charges', 'Charges / pré-état daté', 2, num(doc.charges_annuelles) || num(m?.charges_annuelles) || num(m?.charges)),

...
function slCalcul(d, n) {
  ...
  const refText = (C.dvfM2 && C.annoncesM2) ? `70 % DVF (${fmtNum(C.dvfM2)}) + 30 % annonces (${fmtNum(C.annoncesM2)})`
    : (C.dvfM2 ? "d'après les ventes réelles (DVF)" : "d'après les annonces comparables");
  return section('#F5EDE5',
...
export function buildAvisData(m) {
  ...
  const refM2 = (dvfM2 && annoncesM2) ? Math.round(0.7 * dvfM2 + 0.3 * annoncesM2) : (dvfM2 || annoncesM2 || 0);
  let calcul = null;
  if (refM2 && surface) {
    ...
    calcul = {
      refM2, dvfM2, annoncesM2, surface,
  ...
  // quand il existe : marché = valeur retenue, plancher −10 %, coup de cœur +10 %.
  const central = calcul ? calcul.finale : round5k(num(preco.prix_marche) || num(m2v.valeur_centrale?.valeur_totale));
  const bas = calcul ? round5k(calcul.finale * 0.9) : round5k(num(preco.prix_plancher) || num(m2v.valeur_basse?.valeur_totale));
  const haut = calcul ? round5k(calcul.finale * 1.1) : round5k(num(preco.prix_coup_de_coeur) || num(m2v.valeur_haute?.valeur_totale));

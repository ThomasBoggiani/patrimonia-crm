    ...
    `${header('Caractéristiques', C.title || 'Le bien en détail')}
     <div style="flex:1;display:grid;grid-template-columns:1fr 1.1fr;gap:50px;padding-top:16px;align-items:start">
       ...
       <div>${C.rows.map(r => row(r[0], r[1])).join('')}
         ${dossierStrip}
         ${(C.atouts && C.atouts.length) ? `<div style="margin-top:20px;display:flex;flex-direction:column;gap:8px">${C.atouts.slice(0, 6).map(a => `<div style="display:flex;gap:12px;align-items:flex-start"><img src="${A}/logo-emblem-green.png" style="height:22px;opacity:.55;margin-top:2px"/><div style="font-size:18px;line-height:1.4;color:#241912">${esc(a)}</div></div>`).join('')}</div>` : ''}</div></div>
     ${footer(d, n)}`);
}
...
function slDocuments(d, n) {
  ...
  // Taxe / charges / lots sont désormais sur « Le bien en détail » ; ici on ne
  // garde que le fonds travaux (spécifique dossier).
  const chips = [[D.fondsTravaux, 'Fonds travaux']].filter(x => x[0]);
  ...
  return section('#F5EDE5',
    `${header('Calcul de la valeur', 'Comment nous arrivons à cette valeur')}
     <div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding-top:6px">
       ...
       ${d.pitch ? `<div style="margin:18px 0 2px;padding:16px 22px;background:#FCFAF5;border-left:3px solid #8C9978">
         <div style="font-size:12px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#8C9978;margin-bottom:5px">Ce bien en particulier</div>
         <div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:21px;line-height:1.5;color:#241912">${esc(d.pitch.length > 300 ? d.pitch.slice(0, 298).trim() + '…' : d.pitch)}</div></div>` : ''}
       ${C.steps.length

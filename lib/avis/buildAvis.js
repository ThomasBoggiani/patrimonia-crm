...
function slCaracteristiques(d, n) {
  ...
  const pitchBlk = d.pitch ? `<div style="margin-top:10px;padding:14px 22px;background:#FCFAF5;border-left:3px solid #8C9978"><div style="font-size:11px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#8C9978;margin-bottom:4px">Ce bien en particulier</div><div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:21px;line-height:1.5;color:#241912">${esc(d.pitch.length > 320 ? d.pitch.slice(0, 318).trim() + '…' : d.pitch)}</div></div>` : '';
  return section('#F5EDE5',
    `${header('Caractéristiques', C.title || 'Le bien en détail')}
     ...
     <div style="flex:1;display:grid;grid-template-columns:1fr 1.1fr;gap:50px;padding-top:16px;align-items:start">
       <div style="border:1px solid rgba(41,64,41,.2);background:#FCFAF5;overflow:hidden;position:relative">${C.photo ? `<img src="${esc(C.photo)}" alt="" style="width:100%;height:auto;max-height:820px;object-fit:contain;display:block"/>` : `<div style="width:100%;height:420px;background:#EDE6DC"></div>`}

...
// Étage 0 = rez-de-chaussée (texte complet en détail, « RDC » sur une carte).
const etageLabel = (e) => Number(e) === 0 ? 'Rez-de-chaussée' : String(e);
const etageCourt = (e) => Number(e) === 0 ? 'RDC' : String(e);
...
function slGalerie(d, n) {
  ...
  // Grille toujours « pleine » : 6 si ≥6 photos, sinon 4 (on ne montre pas 5,
  // qui laisserait une case vide). 1–4 gardent leur nombre.
  const dispo = d.galerie.length;
  const target = dispo >= 6 ? 6 : dispo === 5 ? 4 : dispo;
  const g = d.galerie.slice(0, target);
  // Photos qui REMPLISSENT leur case (taille uniforme, sans vide autour) : cadrage
  // « couverture » comme dans une plaquette pro.
  const cell = (p) => `<div style="position:relative;min-width:0;min-height:0;border:1px solid rgba(41,64,41,.18);overflow:hidden;background:#EDE6DC">
    ${p.url ? `<img src="${esc(p.url)}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block"/>` : `<div style="position:absolute;inset:0;background:#EDE6DC"></div>`}
...
function slCaracteristiques(d, n) {
  ...
  // Pitch = « notes » sur le bien : on lui donne de la place (jusqu'à ~700 car.),
  // en pleine largeur au-dessus, pour qu'il ne soit plus coupé.
  const pitchTxt = d.pitch ? (d.pitch.length > 700 ? d.pitch.slice(0, 698).trim() + '…' : d.pitch) : '';
  const pitchBlk = pitchTxt ? `<div style="margin-top:12px;padding:18px 26px;background:#FCFAF5;border-left:4px solid #8C9978"><div style="font-size:12px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#8C9978;margin-bottom:6px">Ce bien en particulier</div><div style="font-family:'Cormorant Garamond',serif;font-style:italic;font-size:23px;line-height:1.55;color:#241912">${esc(pitchTxt)}</div></div>` : '';
  return section('#F5EDE5',
    `${header('Caractéristiques', C.title || 'Le bien en détail')}
     ${pitchBlk}
     <div style="flex:1;display:grid;grid-template-columns:0.85fr 1.15fr;gap:46px;padding-top:16px;align-items:start;min-height:0">
       <div style="height:460px;border:1px solid rgba(41,64,41,.2);background:#FCFAF5;overflow:hidden;position:relative">${C.photo ? `<img src="${esc(C.photo)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"/>` : `<div style="width:100%;height:100%;background:#EDE6DC"></div>`}
         ${C.photoLegende ? `<div style="position:absolute;left:0;right:0;bottom:0;padding:20px 18px 12px;background:linear-gradient(to top,rgba(36,25,18,.65),transparent);color:#F5EDE5;font-size:15px">${esc(C.photoLegende)}</div>` : ''}</div>
...
export function buildAvisData(m) {
  ...
  if (m?.etage != null && m?.etage !== '') cards.push({ figure: `${esc(etageCourt(m.etage))}`, label: 'Étage', desc: '' });
  ...
  if (m?.etage != null && m?.etage !== '') cr.push(['Étage', etageLabel(m.etage)]);
  ...
  // Phase AUTOMATIQUE (pas de choix manuel) : tant que la visite n'est pas
  // renseignée = pré-avis ; dès qu'une observation de visite existe = avis
  // définitif. Les deux documents sont IDENTIQUES, seul le contenu issu de la
  // visite s'ajoute. `av.phase` (ancien réglage manuel) est volontairement ignoré.
  ...
  const visiteRempli = !!av.visite && (
    VIS_KEYS.some(k => String(av.visite[k] || '').trim()) ||
    (Array.isArray(av.visite.prestations) && av.visite.prestations.some(x => String(x || '').trim()))
  );
  const phase = visiteRempli ? 'definitif' : 'pre_avis';
  ...
  const immeuble = (visiteOn && (vis.architecture || vis.immeuble_qualite || vis.parties_communes)) ? {
    // Photo de l'IMMEUBLE : on privilégie l'extérieur (Street View / carte) pour
    // bien montrer le bâtiment, plutôt qu'une photo d'intérieur.
    photo: streetView || mapUrl || photos[0],
  ...
  // Il faut au moins 2 observations pour justifier un slide dédié (sinon une
  // seule ligne — ex. « exposition sud » — donnerait une page quasi vide).
  const habitabilite = habCards.length >= 2 ? { cards: habCards } : null;
...
// ── Briques de charte réexportées ───────────────────────────────────
// Purement additif : permet à la plaquette (lib/avis/buildPlaquette.js) de
// réutiliser EXACTEMENT la même charte que l'avis, sans dupliquer les styles.
// N'affecte en rien le rendu de l'avis lui-même.
export { esc, num, fmtNum, fmtEUR, fmtM2, round5k, etageLabel, etageCourt, A, SECT, EMBLEM, header, section };

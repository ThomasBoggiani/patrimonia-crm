...
function slGalerie(d, n) {
  ...
  const g = d.galerie;
  const slot = (url, cap, hero) => `<div style="${hero ? 'grid-row:1 / -1;' : ''}position:relative;min-width:0;min-height:0;border:1px solid rgba(41,64,41,.18);overflow:hidden;background:#EDE6DC;display:flex;align-items:center;justify-content:center">
    ${url ? `<img src="${esc(url)}" alt="" style="max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;display:block"/>` : `<div style="position:absolute;inset:0;background:#EDE6DC"></div>`}
    ${cap ? `<div style="position:absolute;left:0;right:0;bottom:0;padding:${hero ? '26px 18px 12px' : '22px 16px 10px'};background:linear-gradient(to top,rgba(36,25,18,.62),transparent);color:#F5EDE5;font-size:${hero ? '15px' : '14px'};font-weight:500;letter-spacing:.04em;pointer-events:none">${esc(cap)}</div>` : ''}</div>`;
  const [first, ...rest] = g;
  const petits = rest.slice(0, 4);
  // Layout robuste selon le nombre de photos
  let grid, body;
  if (petits.length === 0) {            // 1 photo → hero plein
    grid = 'grid-template-columns:1fr;grid-template-rows:1fr';
    body = slot(first.url, first.cap, true);
  } else {                              // hero (gauche) + 1 à 4 vignettes (droite)
    const rightCols = petits.length <= 2 ? 1 : 2;
    grid = `grid-template-columns:1.5fr ${'1fr '.repeat(rightCols).trim()};grid-template-rows:repeat(${Math.max(1, Math.ceil(petits.length / rightCols))},1fr)`;
    body = slot(first.url, first.cap, true) + petits.map(p => slot(p.url, p.cap)).join('');
  }
  return section('#F5EDE5',
    `${header('Le bien en images', d.galerieTitle || 'Le bien en images')}
     <div style="flex:1;display:grid;${grid};gap:16px;padding-top:24px">${body}</div>
     ${footer(d, n)}`);
}
...
function slLocalisation(d, n) {
  ...
  const cadastreBloc = L.cadastreRef ? `<div style="margin-bottom:22px;display:flex;align-items:center;gap:12px">
    <span style="display:inline-flex;align-items:center;background:#294029;color:#F5EDE5;font-size:16px;font-weight:700;letter-spacing:.04em;padding:7px 14px;border-radius:6px">Cadastre ${esc(L.cadastreRef)}</span>
    ${L.cadastreSurface ? `<span style="font-size:17px;color:#8C9978">${esc(L.cadastreSurface)}</span>` : ''}</div>` : '';
  return section('#F5EDE5',
    `${header('Le secteur', L.title || "Le quartier et l'accès au bien")}
     <div style="flex:1;display:grid;grid-template-columns:1.4fr 1fr;gap:44px;padding-top:14px;align-items:start">
       ...
       <div>${cadastreBloc}${L.commentaire ? `<div style="font-size:19px;line-height:1.5;color:#241912;margin-bottom:22px">${esc(L.commentaire)}</div>` : ''}
         ${transportsBloc}${grp('Vie de quartier — commerces & écoles', L.commodites)}${grp('Nature & loisirs', L.loisirs)}</div></div>
     ${footer(d, n)}`);
}
...
// Urbanisme & cadastre — parcelle + risques naturels (georisques).
function slUrbanisme(d, n) {
  if (!d.urbanisme) return '';
  const U = d.urbanisme;
  const chip = (t) => `<div style="display:inline-flex;align-items:center;gap:8px;background:#FCFAF5;border:1px solid rgba(41,64,41,.18);border-left:3px solid #8C9978;padding:12px 18px;font-size:18px;color:#241912">${esc(t)}</div>`;
  return section('#F5EDE5',
    `${header('Urbanisme & cadastre', U.title || 'Cadastre, urbanisme & risques')}
     <div style="flex:1;display:grid;grid-template-columns:1.7fr 1fr;gap:46px;padding-top:14px;align-items:stretch">
       <div style="border:1px solid rgba(41,64,41,.2);overflow:hidden;background:#EDE6DC;min-height:760px">${U.image ? `<img src="${esc(U.image)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"/>` : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#8C9978">Plan cadastral</div>`}</div>
       <div style="display:flex;flex-direction:column;justify-content:center">
         ${U.cadastreRef ? `<div style="font-size:15px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#8C9978">Parcelle cadastrale</div>
           <div style="font-family:'Cormorant Garamond',serif;font-size:66px;font-weight:500;color:#294029;line-height:1.02;margin:8px 0 4px">${esc(U.cadastreRef)}</div>
           ${U.contenance ? `<div style="font-size:19px;color:#8C9978;margin-bottom:26px">${esc(U.contenance)}</div>` : '<div style="height:26px"></div>'}` : ''}
         ${U.commentaire ? `<div style="font-size:18px;line-height:1.5;color:#241912;margin-bottom:24px">${esc(U.commentaire)}</div>` : ''}
         ${U.risques && U.risques.length ? `<div style="font-size:14px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:#8C9978;margin-bottom:12px">Risques recensés (Géorisques)</div><div style="display:flex;flex-direction:column;gap:9px">${U.risques.slice(0, 6).map(chip).join('')}</div>` : `<div style="font-size:17px;color:rgba(36,25,18,.6);font-style:italic">Aucun risque naturel majeur recensé à l'adresse.</div>`}
       </div></div>
     ${footer(d, n)}`);
}

// Évolution des prix — médiane €/m² par année (5 ans) + focus 12 derniers mois.
function slEvolutionPrix(d, n) {
  if (!d.evolution || !d.evolution.parAnnee.length) return '';
  const E = d.evolution;
  const maxV = Math.max(...E.parAnnee.map(a => a.m2Median || 0), 1);
  const bar = (a) => {
    const h = Math.round(60 + 300 * ((a.m2Median || 0) / maxV));
    return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:flex-end;flex:1;gap:12px">
      <div style="font-family:'Cormorant Garamond',serif;font-size:26px;color:#F5EDE5">${fmtNum(a.m2Median)}</div>
      <div style="width:64%;max-width:120px;height:${h}px;background:linear-gradient(180deg,#C3CCB8,#8C9978);border-radius:4px 4px 0 0"></div>
      <div style="font-size:17px;color:rgba(245,237,229,.8)">${esc(a.annee)}</div>
      <div style="font-size:13px;color:rgba(245,237,229,.5)">${a.count} vente(s)</div></div>`;
  };
  const focus = E.focus12;
  return `<section style="${SECT('#294029', 'color:#F5EDE5')}">
    ${header('Analyse de marché', E.title || 'L\'évolution des prix sur 5 ans', true)}
    <div style="flex:1;display:grid;grid-template-columns:1.7fr 1fr;gap:56px;align-items:center;padding-top:8px">
      <div style="display:flex;align-items:flex-end;gap:20px;height:440px">${E.parAnnee.map(bar).join('')}</div>
      <div style="border-left:1px solid rgba(245,237,229,.2);padding-left:44px">
        <div style="font-size:15px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#C3CCB8">Focus 12 derniers mois</div>
        <div style="margin-top:16px;font-family:'Cormorant Garamond',serif;font-size:64px;font-weight:500;color:#F5EDE5;line-height:1">${focus && focus.medianeM2 ? fmtNum(focus.medianeM2) + ' €/m²' : '—'}</div>
        ${focus && Number.isFinite(focus.variationPct) && focus.variationPct !== 0 ? `<div style="margin-top:14px;font-size:22px;color:${focus.variationPct > 0 ? '#C3CCB8' : '#CFBA9E'}">${focus.variationPct > 0 ? '▲' : '▼'} ${Math.abs(focus.variationPct)} % vs période précédente</div>` : ''}
        ${focus && focus.count ? `<div style="margin-top:10px;font-size:17px;color:rgba(245,237,229,.66)">Sur ${focus.count} vente(s) enregistrée(s).</div>` : ''}
        ${E.note ? `<div style="margin-top:24px;font-size:16.5px;line-height:1.5;color:rgba(245,237,229,.66)">${esc(E.note)}</div>` : ''}
      </div></div>
    ${footer(d, n, true)}</section>`;
}
...
// Le dossier du bien — DPE compact (bandeau) + copro / fiscalité / diagnostics.
function slDocuments(d, n) {
  if (!d.documents && !d.dpe) return '';
  const D = d.documents || {};
  const P = d.dpe;
  // Bandeau DPE compact : classe en grand + mini-échelle A→G + impact.
  const miniScale = P ? 'ABCDEFG'.split('').map(l => `<div style="width:30px;height:30px;border-radius:3px;background:${DPE_COLORS[l]};color:#fff;display:flex;align-items:center;justify-content:center;font-family:'Cormorant Garamond',serif;font-size:17px;font-weight:600;${l === P.classe ? 'box-shadow:0 0 0 3px #241912' : 'opacity:.5'}">${l}</div>`).join('') : '';
  const dpeBand = P ? `<div style="display:flex;align-items:center;gap:26px;background:#FCFAF5;border:1px solid rgba(41,64,41,.15);border-left:6px solid ${P.color};padding:22px 28px;margin-bottom:26px">
    <div style="text-align:center;min-width:100px"><div style="font-family:'Cormorant Garamond',serif;font-size:62px;font-weight:600;color:${P.color};line-height:.9">${P.classe}</div>${P.conso ? `<div style="font-size:13px;color:#8C9978;margin-top:4px">${fmtNum(P.conso)} kWh/m²·an</div>` : ''}</div>
    <div style="flex:1">
      <div style="font-size:14px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:#8C9978;margin-bottom:5px">DPE — ${esc(P.niveau)}</div>
      <div style="font-size:17px;line-height:1.45;color:#241912">${esc(P.impact)}</div>
      ${P.obligations.length ? `<div style="font-size:14.5px;color:#521C14;margin-top:7px"><b>Obligations :</b> ${P.obligations.map(esc).join(' · ')}</div>` : ''}
      ${P.pistes.length ? `<div style="font-size:14.5px;color:rgba(36,25,18,.6);margin-top:4px"><b>Pistes :</b> ${P.pistes.map(esc).join(' · ')}</div>` : ''}
      ${P.commentaire ? `<div style="font-size:14.5px;color:rgba(36,25,18,.6);font-style:italic;margin-top:4px">${esc(P.commentaire)}</div>` : ''}
    </div>
    <div style="display:flex;gap:6px;flex-shrink:0">${miniScale}</div></div>` : '';
  ...
  const chips = [[D.taxeFonciere, 'Taxe foncière / an'], [D.charges, 'Charges copro / an'], [D.fondsTravaux, 'Fonds travaux'], [D.nbLots, 'Lots']].filter(x => x[0]);
  const stats = chips.length ? `<div style="display:grid;grid-template-columns:repeat(${chips.length},1fr);gap:18px;margin-bottom:24px">${chips.map(x => stat(x[0], x[1])).join('')}</div>` : '';
...
// Calcul transparent de la valeur : médiane DVF × surface, puis chaque
// bonus/malus chiffré et justifié, jusqu'à la valeur retenue.
function slCalcul(d, n) {
  if (!d.calcul) return '';
  const C = d.calcul;
  const col = 'grid-template-columns:2.4fr .8fr 1.15fr 1.2fr';
  const row = (s) => `<div style="display:grid;${col};column-gap:20px;align-items:baseline;padding:13px 0;border-top:1px solid rgba(41,64,41,.13)">
    <div><div style="font-size:19px;color:#241912">${esc(s.label)}</div>${s.note ? `<div style="font-size:14.5px;color:rgba(36,25,18,.6);margin-top:2px">${esc(s.note)}</div>` : ''}</div>
    <div style="text-align:right;font-size:18px;font-weight:600;color:${s.pct >= 0 ? '#294029' : '#521C14'}">${s.pct > 0 ? '+' : ''}${s.pct} %</div>
    <div style="text-align:right;font-size:18px;color:${s.effet >= 0 ? '#294029' : '#521C14'}">${s.effet > 0 ? '+' : ''}${fmtNum(s.effet)} €</div>
    <div style="text-align:right;font-size:18px;color:rgba(36,25,18,.72);font-variant-numeric:tabular-nums">${fmtNum(s.cumule)} €</div></div>`;
  return section('#F5EDE5',
    `${header('Calcul de la valeur', 'Comment nous arrivons à cette valeur')}
     <div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding-top:6px">
       <div style="display:flex;justify-content:space-between;align-items:baseline;background:#FCFAF5;border:1px solid rgba(41,64,41,.15);padding:20px 26px">
         <div><div style="font-size:15px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:#8C9978">Point de départ — médiane DVF du secteur</div>
           <div style="font-size:17px;color:rgba(36,25,18,.7);margin-top:4px">${fmtNum(C.refM2)} €/m² × ${fmtNum(C.surface)} m²</div></div>
         <div style="font-family:'Cormorant Garamond',serif;font-size:44px;color:#294029">${fmtNum(C.ref)} €</div></div>
       ${C.steps.length
        ? `<div style="display:grid;${col};column-gap:20px;font-size:13px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:#8C9978;padding:16px 0 2px"><div>Ajustement — bonus / malus</div><div style="text-align:right">%</div><div style="text-align:right">Effet</div><div style="text-align:right">Valeur</div></div>${C.steps.map(row).join('')}`
        : `<div style="padding:16px 0;font-size:17px;color:rgba(36,25,18,.6);font-style:italic">Aucun ajustement retenu : le bien se situe dans la moyenne du secteur.</div>`}
       <div style="margin-top:16px;display:flex;justify-content:space-between;align-items:center;background:#294029;color:#F5EDE5;padding:24px 28px;position:relative;overflow:hidden">
         ...
         <div style="position:relative"><div style="font-size:15px;font-weight:600;letter-spacing:.16em;text-transform:uppercase;color:#C3CCB8">Valeur de marché retenue${C.totalPct ? ` — ${C.totalPct > 0 ? '+' : ''}${C.totalPct} % au total` : ''}</div>
           ${C.finaleM2 ? `<div style="font-size:16px;color:rgba(245,237,229,.72);margin-top:4px">soit ${fmtNum(C.finaleM2)} €/m²</div>` : ''}</div>
         <div style="font-family:'Cormorant Garamond',serif;font-size:56px;font-weight:600;position:relative">${fmtNum(C.finale)} €</div></div>
       <div style="margin-top:14px;font-size:14.5px;color:rgba(36,25,18,.55)">Chaque ajustement est explicité ci-dessus ; aucun chiffre n'apparaît sans justification. Valeur nette vendeur, hors honoraires.</div>
     </div>
     ${footer(d, n)}`, 'padding:76px 104px 54px');
}
...
export function buildAvisData(m) {
  ...
  const localisation = (mapUrl || transports.length || commodites.length || av.localisation?.commentaire) ? {
    mapUrl, cadastreRef, cadastreSurface, transports,
  ...
  const descCourt = String(m?.description || '').trim();
  const caracteristiques = cr.length ? { rows: cr, photo: caracPhoto, photoLegende: (caracPhoto === streetView) ? "L'immeuble" : '', description: descCourt.length > 180 ? descCourt.slice(0, 178) + '…' : descCourt, title: 'Le bien en détail' } : null;
  ...
  const dvfM2 = num(comp.mediane_m2) || medArr(ventes.map(v => v.prixM2));
  ...
  // URBANISME & CADASTRE — parcelle + risques naturels (georisques, forme variable).
  const rq = m?.risques_data || {};
  const risques = [];
  const pushRisque = (t) => { const s = String(t || '').trim(); if (s && !risques.includes(s)) risques.push(s); };
  // Formes connues : {risquesNaturels:{alea:{present,libelle}}} ou tableaux d'objets à libellé.
  const scanRisques = (obj, depth = 0) => {
    if (!obj || depth > 4) return;
    if (Array.isArray(obj)) { obj.forEach(o => scanRisques(o, depth + 1)); return; }
    ...
    if (present && lib) pushRisque(lib);
    Object.values(obj).forEach(v => scanRisques(v, depth + 1));
  };
  scanRisques(rq);
  const urbanisme = (cadastreRef || risques.length || m?.cadastre_image_url) ? {
    image: m?.cadastre_image_url || m?.satellite_image_url || null,
    cadastreRef, contenance: cadastreSurface,
    risques,
    commentaire: av.localisation?.commentaire_urbanisme || '',
    title: 'Cadastre, urbanisme & risques',
  } : null;
  ...
  // SYNTHÈSE MARCHÉ — réalisé (DVF) vs affiché (annonces).
  const annoncesM2 = medArr(biensSimilaires.map(b => b.prixM2).filter(Boolean));
  ...
  const taxeF = num(doc.taxe_fonciere) || num(m?.taxe_fonciere);
  const documents = (taxeF || num(doc.charges_annuelles) || num(doc.fonds_travaux) || num(doc.copro_nb_lots)
    || doc.travaux_votes || doc.procedures || doc.diagnostics || doc.servitudes || doc.commentaire) ? {
    taxeFonciere: taxeF ? fmtEUR(taxeF) : '',
    charges: num(doc.charges_annuelles) ? fmtEUR(doc.charges_annuelles) : '',
    fondsTravaux: num(doc.fonds_travaux) ? fmtEUR(doc.fonds_travaux) : '',
    nbLots: num(doc.copro_nb_lots) ? fmtNum(doc.copro_nb_lots) : '',
  ...
  // CALCUL TRANSPARENT — médiane DVF × surface, puis chaque bonus/malus chiffré.
  let calcul = null;
  if (dvfM2 && surface) {
    const ref = Math.round(dvfM2 * surface);
    const ajs = (preco.ajustements || []).filter(a => a && (a.label || a.pct));
    let cumule = ref;
    const steps = ajs.map(a => {
      const pct = +a.pct || 0;
      const effet = Math.round(ref * pct / 100); // additif sur la référence (transparent)
      cumule += effet;
      return { label: a.label || 'Ajustement', pct, effet, note: a.note || '', cumule };
    });
    const totalPct = ajs.reduce((s, a) => s + (+a.pct || 0), 0);
    const finale = ref + steps.reduce((s, x) => s + x.effet, 0);
    calcul = { refM2: dvfM2, surface, ref, steps, totalPct, finale, finaleM2: surface ? Math.round(finale / surface) : 0 };
  }
  ...
  return {
    ...
    calcul,
    confiance,
    valorisation: prix.length ? { prix, intro: '', reco: preco.recommandation || '', decote: preco.facteurs_decote || '', positionnement: preco.positionnement || '', ajustements: (preco.ajustements || []).filter(a => a && (a.label || a.pct)), confiance, isPreAvis } : null,
    vigilance: vig,
    etapes: null,
  };
}
...
export function buildAvisHtml(m) {
  ...
  return [
    ...
    slLocalisation(d, N()),        // quartier, commerces, transports
    slRue(d, N()),                 // analyse de la rue (DVF même voie)
    slUrbanisme(d, N()),           // cadastre + urbanisme + risques
    // Le marché — entonnoir marché : local → évolution → comparables → annonces → synthèse
    slMarche(d, N()),              // prix moyens du secteur
    slEvolutionPrix(d, N()),       // évolution 5 ans + focus 12 mois
    ...
    slGalerie(d, N()),
    // Dossier (option « sur pièces ») — DPE compact + copropriété/fiscalité/diagnostics
    slDocuments(d, N()),

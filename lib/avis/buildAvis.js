...
// Pastille de ligne de transport (couleur officielle)
function ligneBadge(line, mode) {
  const bg = getLineColor(line, mode) || '#8C9978';
  const fg = LINE_TEXT_COLORS[String(line).trim()] || '#fff';
  return `<span style="display:inline-block;min-width:26px;text-align:center;background:${bg};color:${fg};font-size:14px;font-weight:700;padding:2px 7px;border-radius:4px;margin-right:5px">${esc(line)}</span>`;
}
...
function slGalerie(d, n) {
  ...
  const g = d.galerie;
  const slot = (url, cap, hero) => `<div style="${hero ? 'grid-row:1 / -1;border:8px solid #294029;box-shadow:0 6px 22px rgba(41,64,41,.25);' : 'border:1px solid rgba(41,64,41,.18);'}position:relative;min-width:0;min-height:0;overflow:hidden">
    ...
    ${hero ? `<div style="position:absolute;top:14px;left:14px;background:#294029;color:#F5EDE5;font-size:12px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;padding:5px 12px">Photo principale</div>` : ''}
...
function slComparablesTable(d, n) {
  ...
  const evo = (T.parAnnee || []).length ? `<div style="margin-top:22px;display:flex;gap:14px;flex-wrap:wrap">${T.parAnnee.map(a => `<div style="background:#FCFAF5;border:1px solid rgba(41,64,41,.13);padding:12px 20px"><span style="font-family:'Cormorant Garamond',serif;font-size:30px;color:#294029">${fmtNum(a.m2Median)} €/m²</span> <span style="font-size:15px;color:#8C9978">${esc(a.annee)} · ${a.count} vente(s)</span></div>`).join('')}</div>` : '';
  return section('#F5EDE5',
    `${header('Comparables', 'Ventes récentes du secteur (DVF)')}
     <div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding-top:6px">${head}${rows}${evo}
...
function slLocalisation(d, n) {
  ...
  return section('#F5EDE5',
    `${header('Le secteur', L.title || "Le quartier et l'accès au bien")}
     <div style="flex:1;display:grid;grid-template-columns:1.35fr 1fr;gap:40px;padding-top:14px;align-items:start">
       <div style="display:flex;flex-direction:column;gap:14px">
         <div style="height:${L.cadastreUrl ? '470px' : '700px'};border:1px solid rgba(41,64,41,.2);overflow:hidden">${L.mapUrl ? `<img src="${esc(L.mapUrl)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"/>` : `<div style="width:100%;height:100%;background:#EDE6DC;display:flex;align-items:center;justify-content:center;color:#8C9978">Carte de situation</div>`}</div>
         ${L.cadastreUrl ? `<div style="height:216px;border:1px solid rgba(41,64,41,.2);overflow:hidden"><img src="${esc(L.cadastreUrl)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block"/></div><div style="font-size:13px;letter-spacing:.12em;text-transform:uppercase;color:#8C9978">Extrait cadastral · parcelle</div>` : ''}
       </div>
       <div>${L.commentaire ? `<div style="font-size:20px;line-height:1.55;color:#241912;margin-bottom:22px">${esc(L.commentaire)}</div>` : ''}
...
export function buildAvisData(m) {
  ...
  const localisation = (mapUrl || transports.length || commodites.length || av.localisation?.commentaire) ? {
    mapUrl, cadastreUrl: m?.cadastre_image_url || null, transports,
  ...
  return {
    ...
    comparablesTable,
    biensSimilaires,
    valorisation: prix.length ? { prix, intro: '', reco: preco.recommandation || '', decote: preco.facteurs_decote || '', positionnement: preco.positionnement || '', ajustements: (preco.ajustements || []).filter(a => a && (a.label || a.pct)) } : null,
    vigilance: vig,
    etapes: null,
  };
}
...
export function buildAvisHtml(m) {
  ...
  return [
    ...
    slAvantPropos(d, N()),
    // Agence — condensée
    slQuiSommesNous(d, N()),        // qui sommes-nous + valeurs (1 page)
    slMethode(d, N()),             // comment on estime
    slSupportsReseau(d, N()),      // supports + réseau (1 page)
    // Le bien — dans l'ordre : secteur → appartement → chiffres → marché → prix
    slLocalisation(d, N()),        // secteur : quartier + cadastre + transports
    slCaracteristiques(d, N()),    // focus appartement
    slGalerie(d, N()),
    slSynthese(d, N()),            // chiffres clés + fourchette
    slMarche(d, N()),
    slComparablesTable(d, N()),    // DVF détaillé
    slBiensSimilaires(d, N()),     // 3 biens disponibles
    slValorisation(d, N()),        // préconisation + ajustements ± %
    slEtapes(d, N()),
    slVigilance(d, N()),           // SWOT / vigilance — en dernier
    slContact(d),
  ].filter(Boolean).join('\n');
}

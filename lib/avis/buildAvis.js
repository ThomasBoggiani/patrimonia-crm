...
function slValorisation(d, n) {
  ...
  return section('#F5EDE5',
    `${header('Valorisation', v.title || 'Notre préconisation de prix')}
     ...
     ${v.decote ? `<div style="margin-top:12px;font-size:15px;line-height:1.5;color:rgba(36,25,18,.55)">Facteurs pris en compte : ${esc(v.decote)}.</div>` : ''}
...
export function buildAvisData(m) {
  ...
  return {
    ...
    comparablesTable,
    biensSimilaires,
    valorisation: prix.length ? { prix, intro: '', reco: preco.recommandation || '', decote: preco.facteurs_decote || '', positionnement: preco.positionnement || '' } : null,
    vigilance: vig,
    etapes: null,
  };
}
...
export function buildAvisHtml(m) {
  ...
  return [
    ...
    slGalerie(d, N()),
    slMarche(d, N()),
    slComparablesTable(d, N()),
    slBiensSimilaires(d, N()),
    slValorisation(d, N()),
    slVigilance(d, N()),
    slEtapes(d, N()),
    slContact(d),

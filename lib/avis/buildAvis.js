...
function slGalerie(d, n) {
  ...
  const g = d.galerie;
  const slot = (url, cap, hero) => `<div style="${hero ? 'grid-row:1 / -1;' : ''}position:relative;min-width:0;min-height:0;border:1px solid rgba(41,64,41,.18);overflow:hidden">
    ...
    `${header('Le secteur', L.title || "Le quartier et l'accès au bien")}
     <div style="flex:1;display:grid;grid-template-columns:1.35fr 1fr;gap:40px;padding-top:14px;align-items:start">
       <div style="display:flex;flex-direction:column;gap:14px">
         ...
         ${L.cadastreUrl ? `<div style="height:216px;border:1px solid rgba(41,64,41,.2);overflow:hidden;background:#EDE6DC"><img src="${esc(L.cadastreUrl)}" alt="" style="width:100%;height:100%;object-fit:contain;display:block"/></div><div style="font-size:13px;letter-spacing:.1em;text-transform:uppercase;color:#8C9978">${esc(L.cadastreLabel || 'Extrait cadastral')}</div>` : ''}
       </div>
...
export function buildAvisData(m) {
  ...
  const cadastreLabel = (pc.section || pc.numero)
    ? `Cadastre · ${[pc.prefixe, pc.section].filter(Boolean).join(' ')} n° ${pc.numero || '—'}${pc.contenance ? ` · ${fmtNum(pc.contenance)} m²` : ''}`.replace(/\s+/g, ' ').trim()
    : 'Extrait cadastral · parcelle';
  const localisation = (mapUrl || transports.length || commodites.length || av.localisation?.commentaire) ? {
    mapUrl, cadastreUrl: m?.cadastre_image_url || null, cadastreLabel, transports,

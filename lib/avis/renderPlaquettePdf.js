...
export async function renderPlaquettePdf(mandat, baseUrl, opts = {}) {
  ...
  const res = await fetch('https://api.pdfshift.io/v3/convert/pdf', {
    ...
    body: JSON.stringify({
      ...
      format: 'A4',
      use_print: true,
      // La plaquette charge beaucoup d'images (couverture + photos par étage +
      // galerie + carte + cadastre). Sans ça, PDFShift abandonne au bout de ~30 s
      // (« page took too long to load »). timeout = délai max de chargement, en
      // secondes (sous les 60 s de maxDuration de la route). delay = pause après
      // chargement pour laisser les polices/images se peindre.
      timeout: 50,
      delay: 1000,

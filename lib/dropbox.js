...
export async function listSharedLinkFiles({ accessToken, url, maxFiles = 200 }) {
  ...
  // Resilient : un sous-dossier illisible est ignore (on ne casse pas tout).
  async function listPath(relPath, isRoot) {
    if (files.length >= maxFiles) return;
    let data;
    try {
      data = await page({ path: relPath, shared_link: { url } });
    } catch (e) {
      if (isRoot) throw e; // erreur sur la racine = vraie erreur
      return; // sous-dossier illisible : on l'ignore
    }
    const subfolders = [];
    while (true) {
      ...
      if (data.has_more && data.cursor) {
        try { data = await page({ cursor: data.cursor }, true); }
        catch { break; }
      } else break;
    }
    for (const sf of subfolders) {
      if (files.length >= maxFiles) return;
      await listPath(sf, false);
    }
  }
...
// Si le compte connecte POSSEDE (ou a monte) le dossier du lien partage,
// renvoie son chemin reel dans le Dropbox du compte (ex "/Mandats/xxx"),
// sinon null. Permet ensuite une liste recursive fiable par chemin.
export async function getSharedLinkPath({ accessToken, url }) {
  try {
    ...
    return meta.path_lower || null;
  } catch {
    return null;
  }
}

// Liste recursive des fichiers d'un dossier possede (par chemin). Recursif OK ici.
export async function listFolderByPath({ accessToken, path, maxFiles = 200 }) {
  const files = [];
  async function page(body, isCont) {
    const res = await fetch(`https://api.dropboxapi.com/2/files/list_folder${isCont ? '/continue' : ''}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    });
  ...
  let data = await page({ path: path || '', recursive: true });
  while (true) {
    for (const e of (data.entries || [])) {
      if (e['.tag'] === 'file') {
        files.push({ name: e.name, path: e.path_lower });
        if (files.length >= maxFiles) return files;
      }
    }
    if (data.has_more && data.cursor) data = await page({ cursor: data.cursor }, true);
    else break;
  }
  return files;
}

// Telecharge un fichier possede (par chemin). Renvoie un Buffer.
export async function downloadFileByPath({ accessToken, path }) {
  const res = await fetch('https://content.dropboxapi.com/2/files/download', {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}`, 'Dropbox-API-Arg': apiArg({ path }) },
  });
  if (!res.ok) {
    const t = await res.text();

...
import { Users, UserPlus, Shield, Clock, Mail, X, Check, Loader2, AlertCircle, Trash2, Edit2 } from 'lucide-react';
    ...
    return (
      <div className="p-8 max-w-4xl">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
          ...
          <div className="font-display text-lg font-semibold text-amber-900 mb-1">Accès réservé</div>
          <p className="text-sm text-amber-800">Seuls les administrateurs peuvent gérer l'équipe.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-4xl font-semibold text-ink mb-1">Équipe</h1>
          <p className="text-sage-dark">Gérer les membres de l'agence ayant accès au CRM</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-cream-100">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-ink/70">Dernière connexion</th>
                ...
                <tr key={m.id} className="border-t border-cream hover:bg-cream-50">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-medium text-white flex-shrink-0 ${m.actif ? 'gradient-sage-dark' : 'bg-stone-400'}`}>
                        {(m.prenom?.[0] || '?')}{(m.nom?.[0] || '')}
                      </div>
                      <div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      {m.id !== currentProfile?.id && (m.actif ? (
      ...
      {editing && (
        <EditMemberForm member={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); loadMembers(); }} />
      )}
    </div>
  );
}
...
function EditMemberForm({ member, onClose, onSaved }) {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    setLoading(true);
    await supabase.from('profiles').update({ prenom, nom, role, fonction }).eq('id', member.id);
    setLoading(false);
    onSaved();
  };
  ...
  return (
    <div className="fixed inset-0 bg-ink/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-cream-dark">
          <h2 className="font-display text-xl font-semibold text-ink">Modifier le membre</h2>
          <button onClick={onClose} className="text-stone-500 hover:text-ink"><X className="w-5 h-5" /></button>
        </div>
